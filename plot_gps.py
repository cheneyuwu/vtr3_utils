...
def load_gps_poses(data_dir):
  ...
  for i in range(len(rosbag_dirs)):
    ...
    gps_poses.append({"timestamp": [], "latitude": [], "longitude": [], "altitude": [], "cov": [], "x": [], "y": []})
    ...
    for j in range(len(messages)):
      ...
      if start_gps_coord is None and start_xy_coord is None:
        # Want to plot with the start of the path at (0, 0)
        start_gps_coord = [gps_msg[1].latitude, gps_msg[1].longitude, gps_msg[1].altitude]
        start_xy_coord = [x, y]
    ...
    if num_large_covariance > 0:
      print("Large cov {}: {}, total: {}".format(rosbag_dirs[i], num_large_covariance, len(messages)))
      bad_gps_runs.append(rosbag_dirs[i])

  print("Bad GPS runs: {}".format(bad_gps_runs))

  return gps_poses, gps_good_segments, rosbag_dirs, bad_gps_runs


def plot_data(gps_poses, gps_good_segments, gps_runs, bad_gps_runs, plot_good_gps_only, data_dir):
...
if __name__ == "__main__":
  ...
  parser.add_argument('--only-good', dest='only_good', action='store_true')

  args = parser.parse_args()

  gps_poses, gps_good_segments, gps_runs, bad_gps_runs = load_gps_poses(args.path)

  plot_data(gps_poses, gps_good_segments, gps_runs, bad_gps_runs, args.only_good, args.path)
