...
def load_points(data_dir):
  ...
  for i in range(len(rosbag_dirs)):
    ...
    for j in range(len(messages)):
      ...
      print(points_msg[0], points_msg[1].header)
      timestamps.append(points_msg[0])

    print("Number of point cloud: ", len(timestamps))
    ...
    ax.plot(list(range(len(timestamps) - 1)), [0.2] * (len(timestamps)-1))
    ax.set_xlabel("point cloud message")
    ax.set_ylabel("time diff (seconds)")
